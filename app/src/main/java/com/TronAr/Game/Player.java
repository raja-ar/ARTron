...
//	private final int MAX_LOD_LEVEL = 3;
	...
	public Player(int player_number, float gridSize, Model mesh, HUD hud)
	{
		...
		// Select Colour
		if(player_number ==TronArGame.OWN_PLAYER)
		{
			// Re-init the colour taken array - must now create players sequentially for this to work
			for(colour = 0; colour <TronArGame.MAX_PLAYERS; colour++)
			{
				ColourTaken[colour] = false;
			}

			ColourTaken[TronArGame.mPrefs.PlayerColourIndex()] = true;
			mPlayerColourIndex =TronArGame.mPrefs.PlayerColourIndex();
		}
		else
		{
			while(!done)
	...
	public void doCrashTestPlayer(Player players[])
	{
		...
		for(j = 0; j <TronArGame.mCurrentPlayers; j++)
		{
