...
package com.RaceAr.Game;

import android.util.Log;

import com.RaceAr.Sound.SoundManager;
import com.RaceAr.Video.Explosion;
import com.RaceAr.Video.GLTexture;
import com.RaceAr.Video.HUD;
import com.RaceAr.Video.Lighting;
import com.RaceAr.Video.Model;
import com.RaceAr.Video.Segment;
import com.RaceAr.Video.TrailMesh;
import com.RaceAr.Video.Trails_Renderer;
import com.RaceAr.Video.Vec;
...
//	private final int MAX_LOD_LEVEL = 3;
	...
	public Player(int player_number, float gridSize, Model mesh, HUD hud)
	{
		...
		// Select Colour
		if(player_number == RaceArGame.OWN_PLAYER)
		{
			// Re-init the colour taken array - must now create players sequentially for this to work
			for(colour = 0; colour < RaceArGame.MAX_PLAYERS; colour++)
			{
				ColourTaken[colour] = false;
			}

			ColourTaken[RaceArGame.mPrefs.PlayerColourIndex()] = true;
			mPlayerColourIndex = RaceArGame.mPrefs.PlayerColourIndex();
		}
		else
		{
			while(!done)
		...
		for(int j=0; j < 4; j++)
		{
			...
			if(V != null)
			{
				if(Current.t1 >= 0.0f && Current.t1 < 1.0f && Current.t2 >= 0.0f && Current.t2 < 1.0f)
				{
					...
					if(RaceArGame.mPrefs.PlaySFX())
						SoundManager.playSound(RaceArGame.CRASH_SOUND, 1.0f);

					Log.e("RACEAR", "Wall CRASH");
					break;
				}
			}
		}
	...
	public void doCrashTestPlayer(Player players[])
	{
		...
		for(j = 0; j < RaceArGame.mCurrentPlayers; j++)
		{
