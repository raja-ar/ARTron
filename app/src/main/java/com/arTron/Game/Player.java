...
//	private final int MAX_LOD_LEVEL = 3;
	...
	public Player(int player_number, float gridSize, Model mesh, HUD hud)
	{
		...
		// Select Colour
		if(player_number ==ARTronGame.OWN_PLAYER)
		{
			// Re-init the colour taken array - must now create players sequentially for this to work
			for(colour = 0; colour <ARTronGame.MAX_PLAYERS; colour++)
			{
				ColourTaken[colour] = false;
			}

			ColourTaken[ARTronGame.mPrefs.PlayerColourIndex()] = true;
			mPlayerColourIndex =ARTronGame.mPrefs.PlayerColourIndex();
		}
		else
		{
			while(!done)
		...
		for(int j=0; j < 4; j++)
		{
			...
			if(V != null)
			{
				if(Current.t1 >= 0.0f && Current.t1 < 1.0f && Current.t2 >= 0.0f && Current.t2 < 1.0f)
				{
					...
					if(ARTronGame.mPrefs.PlaySFX())
						SoundManager.playSound(ARTronGame.CRASH_SOUND, 1.0f);
	...
	public void doCrashTestPlayer(Player players[])
	{
		...
		for(j = 0; j <ARTronGame.mCurrentPlayers; j++)
		{
